  ...
  private fun getExpoGradlePluginsFile(settings: Settings): File {
    val expoModulesAutolinkingPath =
      settings.providers.exec { env ->
        ...
        env.commandLine("node", "--print", "const fs = require('fs'); const path = require('path'); const expoRoot = fs.realpathSync(path.dirname(require.resolve('expo/package.json'))); require.resolve('expo-modules-autolinking/package.json', { paths: [expoRoot] })")
