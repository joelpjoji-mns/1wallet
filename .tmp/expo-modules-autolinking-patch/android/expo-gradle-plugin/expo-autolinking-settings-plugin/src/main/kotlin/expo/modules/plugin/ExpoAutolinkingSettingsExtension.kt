  ...
  val reactNativeGradlePlugin: File by lazy {
    File(
      settings.providers.exec { env ->
        ...
        env.commandLine("node", "--print", "const fs = require('fs'); const path = require('path'); const reactNativeRoot = fs.realpathSync(path.dirname(require.resolve('react-native/package.json'))); require.resolve('@react-native/gradle-plugin/package.json', { paths: [reactNativeRoot] })")
