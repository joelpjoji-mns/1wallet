...
The current target release is `1.3.1` with version code `1030100`. Use `1.3.0` as the baseline when testing update detection.

Release asset names are channel-aware and omit `versionCode` for readability:

```text
1wallet-1.3.1-stable-arm64-v8a.apk
1wallet-1.3.1-beta.1-beta-arm64-v8a.apk
```

Keep `versionCode` in Android build metadata, Firestore release IDs, workflow artifact names, and Git tags. Android uses `versionCode` for install ordering, so beta builds must stay above the last stable build and below the next stable build. After stable `1.3.0`, use beta display versions such as `1.3.1-beta.1`; keep beta builds under the current stable minor line until a new stable minor, such as `1.4.0`, has actually shipped.

Recommended GitHub Release tags:

```text
android-stable-v1.3.1-1030100
android-beta-v1.3.1-beta.1-1030001
```
...
```text
appUpdates/android/releases/1030100
```
...
```json
{
  ...
  "apk": {
    "downloadUrl": "https://github.com/joelpjoji-mns/1wallet/releases/download/android-stable-v1.3.1-1030100/1wallet-1.3.1-stable-arm64-v8a.apk",
    "fileName": "1wallet-1.3.1-stable-arm64-v8a.apk",
    "sizeBytes": 30000000,
...
```powershell
pnpm run mobile:update:manifest -- --apk apps/mobile/android/app/build/outputs/apk/release/app-release.apk --version 1.3.1 --version-code 1030100 --url "https://example.com/1wallet-1.3.1-stable-arm64-v8a.apk" --file-name "1wallet-1.3.1-stable-arm64-v8a.apk" --architecture arm64-v8a --channel stable --release-type patch --feature "Home header now shows 1Wallet again" --fix "Update download validation" --note "Android installer confirmation is required" --output importdata/mobile-update-1.3.1-stable.json
```
