...
import type { AppUpdateRelease } from '../src/updates/types';
        ...
        {release ? (
          <>
            <InfoRow
              ...
              tone="positive"
            />
            <InfoRow icon="source-branch" label="New build" value={String(release.versionCode)} />
            <InfoRow
            ...
            <InfoRow
              ...
              value={formatEta(state.download?.etaSeconds ?? release.apk.estimatedDownloadSeconds)}
            />
          </>
        ) : null}
        <InfoRow icon="clock-outline" label="Last checked" value={dateValue(state.lastCheckedAt)} />
      </SectionCard>
      ...
      {state.jsUpdate.available && !release ? (
        <SectionCard title="JavaScript update" compact>
          ...
          <View style={styles.actionRow}>
            {state.jsUpdate.downloaded ? (
              <Button mode="contained" icon="restart" onPress={() => void updates.applyJsUpdate()}>
                Restart app
              </Button>
            ) : (
              <Button
