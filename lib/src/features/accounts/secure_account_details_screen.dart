...
class _SecureAccountDetailsScreenState extends ConsumerState<SecureAccountDetailsScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      print('DEBUG: initState - Account type: ${account.type}, ID: ${account.id}');
      print('DEBUG: initState - encryptedDetails is ${account.encryptedDetails == null ? 'NULL' : 'NOT NULL, length: ${account.encryptedDetails!.length}'}');
          ...
          details.forEach((k, v) {
            try {
              ...
              if (k == 'number' || k == 'account_number') {
                ...
                _customFields.add(MapEntry('Routing Number', TextEditingController(text: decrypted)));
              } else if (k != 'name' && k != 'bank_name') {
                _customFields.add(MapEntry(k, TextEditingController(text: decrypted)));
              }
            } catch (e) {
  ...
  Future<void> _saveSecureDetails(Account account) async {
    ...
    if (isCard) {
      print('DEBUG: Saving card details. Card Number is ${_cardNumberController.text.isNotEmpty ? 'NOT EMPTY' : 'EMPTY'}');
      if (_cardNumberController.text.isNotEmpty) newEncrypted['number'] = encrypter.encrypt(_cardNumberController.text, iv: iv).base64;
      if (_expiryController.text.isNotEmpty) newEncrypted['expiry'] = encrypter.encrypt(_expiryController.text, iv: iv).base64;
      if (_ccvController.text.isNotEmpty) newEncrypted['ccv'] = encrypter.encrypt(_ccvController.text, iv: iv).base64;
      
      if (_cardNumberController.text.length >= 4) {
        newCardLast4 = _cardNumberController.text.substring(_cardNumberController.text.length - 4);
      }
    } else {
      print('DEBUG: Saving bank details. Account Number is ${_accountNumberController.text.isNotEmpty ? 'NOT EMPTY' : 'EMPTY'}');
      if (_accountNumberController.text.isNotEmpty) newEncrypted['account_number'] = encrypter.encrypt(_accountNumberController.text, iv: iv).base64;
      
      if (_accountNumberController.text.length >= 4) {
        newAccountLast4 = _accountNumberController.text.substring(_accountNumberController.text.length - 4);
      }
    }
    
    for (final field in _customFields) {
      if (field.value.text.isNotEmpty) {
        newEncrypted[field.key] = encrypter.encrypt(field.value.text, iv: iv).base64;
      }
    }

    print('DEBUG: newEncrypted map has ${newEncrypted.length} keys: $newEncrypted');
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Secure details saved'), behavior: SnackBarBehavior.floating),
        );
      }
    } catch (e) {
  ...
  Widget _buildFieldWithCopy(TextEditingController controller, String label, [TextInputType? type, int? maxLength, List<TextInputFormatter>? formatters]) {
    return Padding(
        ...
        children: [
            if (isCard)
              CreditCardView(
                ...
                isUnlocked: true,
                customFields: Map.fromEntries(_customFields.where((e) => e.value.text.isNotEmpty).map((e) => MapEntry(e.key, e.value.text))),
              )
            else
              CreditCardView(
                type: 'bank',
            ...
            if (isCard) ...[
              _buildFieldWithCopy(_cardNumberController, 'Card Number', TextInputType.number, 19),
              Row(
                children: [
                  Expanded(child: _buildFieldWithCopy(_expiryController, 'Expiry (MM/YY)', TextInputType.number, 5, [_ExpiryDateFormatter()])),
                  const SizedBox(width: 16),
                  Expanded(child: _buildFieldWithCopy(_ccvController, 'CVV', TextInputType.number, 4)),
                ],
              ),
            ] else ...[
              _buildFieldWithCopy(_accountNumberController, 'Account Number', TextInputType.number),
            ],
            ...
            if (_customFields.isNotEmpty) ...[
              ...
              for (var i = 0; i < _customFields.length; i++)
                Row(
                  children: [
                    Expanded(
                      child: _buildFieldWithCopy(_customFields[i].value, _customFields[i].key),
                    ),
                    IconButton(
...
class _ExpiryDateFormatter extends TextInputFormatter {
  ...
  TextEditingValue formatEditUpdate(
    ...
    } else if (newText.length == 2 && newValue.text.length > oldValue.text.length) {
      newText = '$newText/';
    }
