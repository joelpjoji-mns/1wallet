...
class ExchangeRateService {
  ...
  Future<void> refreshRatesIfStale() async {
    ...
    for (final rate in state.exchangeRates) {
      if (rate.provider != null && DateTime.now().difference(rate.updatedAt ?? rate.asOfDate) > const Duration(minutes: 5)) {
        isStale = true;
        break;
      }
    }
    ...
    for (final c in state.preferences.enabledCurrencies) {
      ...
      final hasRate = state.exchangeRates.any((r) => r.base.toUpperCase() == c.toUpperCase() && r.quote.toUpperCase() == base);
      if (!hasRate) {
        isStale = true;
        break;
      }
  ...
  Future<void> refreshRates() async {
    ...
    try {
      ...
      for (final currency in state.preferences.enabledCurrencies) {
        if (currency.toUpperCase() == state.preferences.baseCurrency.toUpperCase()) continue;
        ...
        if (rates.containsKey(lower)) {
          ...
          final existingIndex = updatedRecords.indexWhere(
            (r) => r.base.toUpperCase() == currency.toUpperCase() && r.quote.toUpperCase() == state.preferences.baseCurrency.toUpperCase()
          );

          if (existingIndex >= 0 && updatedRecords[existingIndex].provider == null) {
      ...
      for (final currency in state.preferences.enabledCurrencies) {
        ...
        if (rates.containsKey(lower)) {
          ...
          final existingIndex = updatedRecords.indexWhere(
            (r) => r.base.toUpperCase() == state.preferences.baseCurrency.toUpperCase() && r.quote.toUpperCase() == currency.toUpperCase()
          );
      ...
      if (changed) {
        await _ref.read(ledgerProvider.notifier).setExchangeRatesDirectly(updatedRecords);
      }
    } catch (e) {
